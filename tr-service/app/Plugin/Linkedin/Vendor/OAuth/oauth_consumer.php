...
class OAuth_Consumer {
	...
	/**
	 * Call API with a raw POST request
	 */
	public function postRaw($accessTokenKey, $accessTokenSecret, $url, $rawContent, $contentType) {
		$accessToken = new OAuthToken($accessTokenKey, $accessTokenSecret);
		$oauthRequest = $this->createRequest('POST', $url, $accessToken, array());
		
		$request = array(
			'header' => array(
				'Authorization' => substr($oauthRequest->to_header(), 15),
				'Content-Type' => $contentType
			)
		);
		return $this->doPost($url, $rawContent, $request);
	}
	...
	private function doGet($url) {
		...
		$this->responseHeader = $socket->response['header'];
		return $result;
	}

	private function doPost($url, $data, $request = null) {
		$socket = new HttpSocket();
		return $socket->post($url, $data, $request);
	}
