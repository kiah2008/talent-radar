...
class LinkedinComponent extends Component {
	 ...
	 * @return array
	 */
	public function getKeyAndSecretOfUser() {
		$accessToken = $this->Session->read($this->sessionAccess);
		$values = array();
		if($accessToken && is_object($accessToken)) {
			$valuesAux = $this->Session->read($this->sessionAccess);
			$values['linkedin_key'] = $valuesAux->key;
			$values['linkedin_secret'] = $valuesAux->secret;
		}
